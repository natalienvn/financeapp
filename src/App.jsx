...
/* ═══════════════════════════════════════════════════════════════════════════
   ALLOCATION ENGINE — every input is a parameter, nothing is hardcoded
   ═══════════════════════════════════════════════════════════════════════════ */
function computeMonth(mi, pay, hBal, tBal, rYTD, cfg) {
  const {
    goals, rent, jhuPerCheck, taxRate, expenses, subs,
    rothPerCheck, rothMayIMSLP, rothMayP1,
    hysaBaseline, hysaRedirectPerCheck, hysaSepIMSLP, hysaSepP1, hysaSepP2,
    travelPerCheck, travelActive,
  } = cfg;

  const brokPer = Math.round((goals.brokerageMonthly / 3) * 100) / 100;

  // ── Roth schedule (editable per-check amounts) ──
  let rI = 0, rP1 = 0, rP2 = 0;
  const rothRemaining = Math.max(0, goals.rothLimit - rYTD);
  if (mi <= 3) {
    rI = Math.min(rothPerCheck.imslp, rothRemaining);
    rP1 = Math.min(rothPerCheck.p1, Math.max(0, rothRemaining - rI));
    rP2 = Math.min(rothPerCheck.p2, Math.max(0, rothRemaining - rI - rP1));
  } else if (mi === 4) {
    rI = Math.min(rothMayIMSLP, rothRemaining);
    rP1 = Math.min(rothMayP1, Math.max(0, rothRemaining - rI));
    rP2 = 0;
  }
  const rothTot = rI + rP1 + rP2;

  // ── HYSA schedule (editable baselines, redirects, Sep amounts) ──
  let hI = 0, hP1 = 0, hP2 = 0;
  const hNeed = Math.max(0, goals.hysaTarget - hBal);
  if (hNeed > 0) {
    if (mi <= 4) {
      hI = Math.min(hysaBaseline, hNeed);
      hP1 = Math.min(hysaBaseline, Math.max(0, hNeed - hI));
      hP2 = Math.min(hysaBaseline, Math.max(0, hNeed - hI - hP1));
    } else if (mi >= 5 && mi <= 7) {
      hI = Math.min(hysaRedirectPerCheck, hNeed);
      hP1 = Math.min(hysaRedirectPerCheck, Math.max(0, hNeed - hI));
      hP2 = Math.min(hysaRedirectPerCheck, Math.max(0, hNeed - hI - hP1));
    } else if (mi === 8) {
      hI = Math.min(hysaSepIMSLP, hNeed);
      hP1 = Math.min(hysaSepP1, Math.max(0, hNeed - hI));
      hP2 = Math.min(hysaSepP2, Math.max(0, hNeed - hI - hP1));
    }
  }
  const hTot = hI + hP1 + hP2;

  // ── Travel (editable per-check, editable cutoff month) ──
  const tAct = mi < travelActive;
  const tI = tAct ? travelPerCheck.imslp : 0;
  const tP1 = tAct ? travelPerCheck.p1 : 0;
  const tP2 = tAct ? travelPerCheck.p2 : 0;
  ...
  return {
    ...
    checks: {
      imslp: { gross: iG, tax: iTx, afterTax: iAT, rent: rent.myShare, jhu: jhuPerCheck, roth: rI, brok: brokPer, hysa: hI, travel: tI, surplus: iSur },
      p1: { gross: p1G, rent: p1R, jhu: jhuPerCheck, roth: rP1, brok: brokPer, hysa: hP1, travel: tP1, surplus: p1Sur },
      p2: { gross: p2G, jhu: jhuPerCheck, roth: rP2, brok: brokPer, hysa: hP2, travel: tP2, cc: ccTot, surplus: p2Sur },
    },
    ...
    <div style={{ flex: 1, minWidth: small ? 70 : 120, ...sx }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        <input type="number" value={value} onChange={e => onChange(parseFloat(e.target.value) || 0)}
          style={{ background: C.bgWarm, border: `1.5px solid ${C.border}`, borderRadius: 10, color: C.text,
            padding: small ? "6px 10px" : "9px 12px", fontFamily: "'Nunito',sans-serif", fontSize: small ? 12 : 13,
            fontWeight: 600, width: "100%", outline: "none", paddingLeft: prefix ? 24 : 12,
            paddingRight: suffix ? 30 : 12, transition: "border-color 0.2s, box-shadow 0.2s" }} />
...
function GoalCard({ title, current, target, color, colorBg, colorBorder, icon, deadline }) {
  ...
  return (
    <div style={{ background: colorBg, border: `1.5px solid ${colorBorder}`, borderRadius: 16, padding: "18px 20px", position: "relative", overflow: "hidden" }}>
...
function CheckCard({ label, data, showTax, showRent, showCC, accent }) {
  const rows = [];
  if (showTax) rows.push({ l: "1099 Tax", v: data.tax, c: C.rose, neg: true });
...
function SectionCard({ children, style: sx }) {
  return <div style={{ background: C.bgCard, border: `1.5px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: "0 2px 12px rgba(61,50,41,0.04)", marginBottom: 16, ...sx }}>{children}</div>;
}
...
export default function BudgetApp() {
  ...
  // ── Starting balances ──
  const [bal, setBal] = useState({ hysaStart: 28300, travelStart: 234, rothContributed: 0 });

  // ── Goals ──
  const [goals, setGoals] = useState({ rothLimit: 7500, hysaTarget: 36000, travelTarget: 2000, brokerageMonthly: 1500, brokerageAge50: 800000, rothAge50: 300000 });

  // ── Rent ──
  const [rent, setRent] = useState({ total: 1700, myShare: 425 });

  // ── JHU per paycheck ──
  const [jhuPerCheck, setJhuPerCheck] = useState(50);

  // ── Tax rate ──
  const [taxRate, setTaxRate] = useState(0.30);

  // ── Roth contribution schedule (editable!) ──
  const [rothPerCheck, setRothPerCheck] = useState({ imslp: 500, p1: 500, p2: 500 });
  const [rothMayIMSLP, setRothMayIMSLP] = useState(750);
  const [rothMayP1, setRothMayP1] = useState(750);

  // ── HYSA contribution schedule (editable!) ──
  const [hysaBaseline, setHysaBaseline] = useState(268);
  const [hysaRedirectPerCheck, setHysaRedirectPerCheck] = useState(500);
  const [hysaSepIMSLP, setHysaSepIMSLP] = useState(500);
  const [hysaSepP1, setHysaSepP1] = useState(500);
  const [hysaSepP2, setHysaSepP2] = useState(120);

  // ── Travel fund (editable per-check and cutoff) ──
  const [travelPerCheck, setTravelPerCheck] = useState({ imslp: 83.33, p1: 83.33, p2: 83.34 });
  const [travelActiveUntil, setTravelActiveUntil] = useState(8); // months 0-7 active = Aug
  ...
  const [brokA, setBrokA] = useState({ FXAIX: 60, FSPSX: 30, FSELX: 10 });
  ...
  // ── Config object fed to engine ──
  const cfg = useMemo(() => ({
    goals, rent, jhuPerCheck, taxRate, expenses: exps, subs,
    rothPerCheck, rothMayIMSLP, rothMayP1,
    hysaBaseline, hysaRedirectPerCheck, hysaSepIMSLP, hysaSepP1, hysaSepP2,
    travelPerCheck, travelActive: travelActiveUntil,
  }), [goals, rent, jhuPerCheck, taxRate, exps, subs, rothPerCheck, rothMayIMSLP, rothMayP1, hysaBaseline, hysaRedirectPerCheck, hysaSepIMSLP, hysaSepP1, hysaSepP2, travelPerCheck, travelActiveUntil]);

  const year = useMemo(() => {
    const r = []; let hB = bal.hysaStart, tB = bal.travelStart, rY = bal.rothContributed;
    for (let i = 0; i < 12; i++) { const m = computeMonth(i, pays[i], hB, tB, rY, cfg); r.push(m); hB = m.hysa.end; tB = m.travel.end; rY = m.roth.ytd; }
    return r;
  }, [pays, bal, cfg]);
  ...
  const tabDef = [
    ...
    { id: "investments", label: "Investments" }, { id: "expenses", label: "Expenses" },
    { id: "settings", label: "Settings" }, { id: "yearly", label: "12-Month" },
  ];
  ...
  const addBtn = { padding: "7px 16px", fontSize: 11, fontFamily: "'Nunito',sans-serif", fontWeight: 700, color: C.sage, background: C.sageBg, border: `1.5px solid ${C.sageBorder}`, borderRadius: 8, cursor: "pointer" };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Nunito',sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        ::-webkit-scrollbar{width:6px;height:6px}::-webkit-scrollbar-track{background:${C.bgAlt}}::-webkit-scrollbar-thumb{background:${C.border};border-radius:3px}
      ...
      <div style={{ padding: "24px 32px", maxWidth: 1120 }}>
        ...
        {tab === "overview" && (<div style={{ animation: "fadeUp 0.4s ease" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 14, marginBottom: 22 }}>
            ...
            <GoalCard title="HYSA" current={cur.hysa.end} target={goals.hysaTarget} color={C.sage} colorBg={C.sageBg} colorBorder={C.sageBorder} icon="🌿" deadline={`${fmtInt(goals.hysaTarget)} by Sep`} />
            <GoalCard title="Travel Fund" current={cur.travel.end} target={goals.travelTarget} color={C.gold} colorBg={C.goldBg} colorBorder={C.goldBorder} icon="☀" deadline={`${fmtInt(goals.travelTarget)} by Aug`} />
            <GoalCard title="Brokerage" current={cur.brok.total} target={goals.brokerageMonthly} color={C.blue} colorBg={C.blueBg} colorBorder={C.blueBorder} icon="◈" deadline={`${fmtInt(goals.brokerageMonthly)}/mo`} />
          </div>

          <SectionCard style={{ background: `linear-gradient(135deg, ${C.bgCard}, ${C.bgWarm})` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(115px,1fr))", gap: 10 }}>
              {[
                ...
                { l: "JHU Loan", v: jhuPerCheck * 3, c: C.textMed, bg: C.bgAlt },
          ...
          {cur.surplus > 0 && <div style={{ background: C.sageBg, border: `1.5px solid ${C.sageBorder}`, borderRadius: 14, padding: "14px 18px" }}>
            ...
            <div style={{ fontSize: 12, color: C.textMed, fontWeight: 500 }}>{cur.hysa.end < goals.hysaTarget ? "→ Route to HYSA" : cur.travel.end < goals.travelTarget && mo < travelActiveUntil ? "→ Route to Travel Fund" : "→ Route to Extra Brokerage (FXAIX 60% / FSPSX 30% / FSELX 10%)"}</div>
          </div>}
          {cur.surplus < 0 && <div style={{ background: C.roseBg, border: `1.5px solid ${C.roseBorder}`, borderRadius: 14, padding: "14px 18px" }}>
            ...
            <div style={{ fontSize: 12, color: C.textMed, fontWeight: 500 }}>Expenses exceed income. Adjust amounts or reduce spending.</div>
          </div>}
        </div>)}
        ...
        {tab === "paychecks" && (<div style={{ animation: "fadeUp 0.4s ease" }}>
          ...
          <SectionCard>
            ...
            <div style={{ fontSize: 12, color: C.textMed, lineHeight: 2, fontWeight: 500 }}>
              <strong style={{ color: C.terracotta }}>IMSLP:</strong> Tax ({(taxRate * 100).toFixed(0)}%) → ¼ Rent → ${jhuPerCheck} JHU → Roth → Brokerage → HYSA → Travel → Surplus<br />
              <strong style={{ color: C.sage }}>Paycheck 1:</strong> ¾ Rent → ${jhuPerCheck} JHU → Roth → Brokerage → HYSA → Travel → Surplus<br />
              <strong style={{ color: C.blue }}>Paycheck 2:</strong> ${jhuPerCheck} JHU → All CC → Roth → Brokerage → HYSA → Travel → Surplus
            </div>
          </SectionCard>
        </div>)}
        ...
        {tab === "investments" && (<div style={{ animation: "fadeUp 0.4s ease" }}>
          ...
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 18 }}>
            <SectionCard style={{ flex: 1, minWidth: 230, borderColor: C.plumBorder, marginBottom: 0 }}>
            ...
            <SectionCard style={{ flex: 1, minWidth: 230, borderColor: C.blueBorder, marginBottom: 0 }}>
          ...
          <SectionCard>
            ...
            <div style={{ fontSize: 12, color: C.textMed, lineHeight: 2, fontWeight: 500 }}>
              ...
              <strong style={{ color: C.blue }}>Brokerage:</strong> {Object.entries(brokA).map(([f, p]) => `${p}% ${f}`).join(" · ")}<br />
          ...
          <SectionCard>
            ...
            {exps.map(e => (
              <div key={e.id} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8, flexWrap: "wrap" }}>
                ...
                <select value={e.cc} onChange={ev => updateExp(e.id, "cc", ev.target.value)} style={{ width: 95 }}><option value="AMEX">AMEX</option><option value="Regular">Regular</option></select>
          ...
          <SectionCard>
            ...
            {subs.map(s => (
              <div key={s.id} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8, flexWrap: "wrap" }}>
                ...
                <select value={s.cc} onChange={ev => updateSub(s.id, "cc", ev.target.value)} style={{ width: 95 }}><option value="AMEX">AMEX</option><option value="Regular">Regular</option></select>
                <button onClick={() => setSubs(p => p.filter(x => x.id !== s.id))} style={rmBtn}>✕</button>
              </div>
            ))}
          </SectionCard>
          <SectionCard style={{ display: "flex", justifyContent: "space-around", flexWrap: "wrap", gap: 16, background: `linear-gradient(135deg,${C.bgCard},${C.bgWarm})` }}>
        ...
        {tab === "settings" && (<div style={{ animation: "fadeUp 0.4s ease" }}>
          <div style={secT}>Settings — Edit Everything</div>
          ...
          <SectionCard>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              <Field label="JHU per Paycheck" value={jhuPerCheck} onChange={setJhuPerCheck} />
              <Field label="1099 Tax Rate" value={Math.round(taxRate * 100)} onChange={v => setTaxRate(v / 100)} prefix="" suffix="%" />
            </div>
          </SectionCard>

          <SectionCard>
            <div style={subT}>Roth IRA Contribution Schedule</div>
            <div style={{ fontSize: 11, color: C.textLight, marginBottom: 10, fontWeight: 500 }}>Jan–Apr: per-paycheck amounts. May: final push amounts.</div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
              <Field label="Jan–Apr: IMSLP" value={rothPerCheck.imslp} onChange={v => setRothPerCheck(p => ({ ...p, imslp: v }))} />
              <Field label="Jan–Apr: Paycheck 1" value={rothPerCheck.p1} onChange={v => setRothPerCheck(p => ({ ...p, p1: v }))} />
              <Field label="Jan–Apr: Paycheck 2" value={rothPerCheck.p2} onChange={v => setRothPerCheck(p => ({ ...p, p2: v }))} />
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <Field label="May: IMSLP" value={rothMayIMSLP} onChange={setRothMayIMSLP} />
              <Field label="May: Paycheck 1" value={rothMayP1} onChange={setRothMayP1} />
              <div style={{ flex: 1, minWidth: 120, display: "flex", alignItems: "flex-end" }}>
                <div style={{ background: C.plumBg, border: `1px solid ${C.plumBorder}`, borderRadius: 10, padding: "9px 14px", width: "100%" }}>
                  <div style={{ fontSize: 10, color: C.textLight, letterSpacing: 1, textTransform: "uppercase", fontWeight: 600 }}>May: P2</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.plum }}>$0 (done)</div>
                </div>
              </div>
            </div>
          </SectionCard>
          ...
          <SectionCard>
            <div style={subT}>HYSA Contribution Schedule</div>
            <div style={{ fontSize: 11, color: C.textLight, marginBottom: 10, fontWeight: 500 }}>Jan–May baseline per check, Jun–Aug redirect per check, Sep final amounts.</div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
              <Field label="Jan–May: per check" value={hysaBaseline} onChange={setHysaBaseline} />
              <Field label="Jun–Aug: per check" value={hysaRedirectPerCheck} onChange={setHysaRedirectPerCheck} />
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <Field label="Sep: IMSLP" value={hysaSepIMSLP} onChange={setHysaSepIMSLP} />
              <Field label="Sep: Paycheck 1" value={hysaSepP1} onChange={setHysaSepP1} />
              <Field label="Sep: Paycheck 2" value={hysaSepP2} onChange={setHysaSepP2} />
            </div>
          </SectionCard>

          <SectionCard>
            <div style={subT}>Travel Fund Schedule</div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
              <Field label="IMSLP per check" value={travelPerCheck.imslp} onChange={v => setTravelPerCheck(p => ({ ...p, imslp: v }))} />
              <Field label="P1 per check" value={travelPerCheck.p1} onChange={v => setTravelPerCheck(p => ({ ...p, p1: v }))} />
              <Field label="P2 per check" value={travelPerCheck.p2} onChange={v => setTravelPerCheck(p => ({ ...p, p2: v }))} />
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
              <div style={{ flex: 1, minWidth: 180 }}>
                <label style={{ fontSize: 10, color: C.textLight, textTransform: "uppercase", letterSpacing: 1.5, display: "block", marginBottom: 5, fontWeight: 500 }}>Active through month</label>
                <select value={travelActiveUntil} onChange={e => setTravelActiveUntil(parseInt(e.target.value))} style={{ width: "100%", padding: "9px 12px" }}>
                  {MONTHS.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
                </select>
              </div>
              <div style={{ flex: 1, minWidth: 120, display: "flex", alignItems: "flex-end" }}>
                <div style={{ background: C.goldBg, border: `1px solid ${C.goldBorder}`, borderRadius: 10, padding: "9px 14px", width: "100%" }}>
                  <div style={{ fontSize: 10, color: C.textLight, letterSpacing: 1, fontWeight: 600 }}>MONTHLY TOTAL</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.gold }}>{fmt(travelPerCheck.imslp + travelPerCheck.p1 + travelPerCheck.p2)}</div>
                </div>
              </div>
            </div>
          </SectionCard>
          ...
          <SectionCard>
            ...
            {(() => { const t = Object.values(brokA).reduce((s, v) => s + v, 0); return t !== 100 ? <div style={{ fontSize: 11, color: C.red, marginTop: 8, fontWeight: 700 }}>⚠ Sum is {t}% — should be 100%</div> : <div style={{ fontSize: 11, color: C.sage, marginTop: 8, fontWeight: 700 }}>✓ Allocations = 100%</div>; })()}
          </SectionCard>
        ...
        {tab === "yearly" && (<div style={{ animation: "fadeUp 0.4s ease" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: "'Nunito',sans-serif" }}>
                ...
                <tbody>{year.map((m, i) => {
                  ...
                  return <tr key={i} onClick={() => { setMo(i); setTab("overview"); }} style={{ borderBottom: `1px solid ${C.borderLight}`, cursor: "pointer", background: i === mo ? C.bgAlt : "transparent", transition: "background 0.15s" }}>
                    ...
                    <td style={{ padding: "10px 8px", textAlign: "right", color: C.textMed, fontWeight: 600 }}>{fmtInt(jhuPerCheck * 3)}</td>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 14 }}>
            <SectionCard style={{ borderColor: C.sageBorder }}>
            ...
            <SectionCard style={{ borderColor: C.goldBorder }}>
              <div style={{ fontSize: 13, color: C.gold, fontWeight: 800, marginBottom: 8 }}>Travel Fund</div>
              {year.filter((_, i) => i < travelActiveUntil).map((m, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", fontSize: 12, fontWeight: 600, color: m.travel.end >= goals.travelTarget ? C.gold : C.textLight }}>
                <span>{m.month}</span><span>{fmtInt(m.travel.end)}{m.travel.end >= goals.travelTarget ? " ✓" : ""}</span>
              </div>)}
            </SectionCard>
            <SectionCard style={{ borderColor: C.plumBorder }}>
